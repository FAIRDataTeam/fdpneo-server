...
- **The SPARQL projection** (`authorized_graphs(subject, read)`): the permitted set is intersected with the state-visible set. Visible = `read_permitted ∩ (published ∪ modify_permitted)`, where `published` is the set of records whose meta state is `PUBLISHED` and `modify_permitted` is the subject's own `authorized_graphs(subject, modify)` (already cached). For anonymous, `modify_permitted` is empty, so the projection collapses to `read_permitted ∩ published` — anonymous SPARQL sees only published graphs.
