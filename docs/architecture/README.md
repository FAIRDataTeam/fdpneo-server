...
10. **Persistent identifiers.** Resolved by [ADR-0014](../adr/0014-persistent-identifiers.md) (v0.3.0): a persistent `identifier_base` (a W3ID/PURL redirector prefix) is decoupled from the serving `base_url`, inbound requests are canonicalized in-server, and a dual model honors client-supplied identifiers (within-base as the subject; foreign as `owl:sameAs`/`dct:identifier`). `fdp pid` automates the W3ID redirect + PR and verifies resolution. Open sub-questions deferred: Handle/DOI minting integrations, and a faithful backup/restore built on the now-stable identifiers.
