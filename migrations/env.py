...
Reads the DSN from :class:`~fdp.config.Settings.postgres_dsn`, so the same
config drives the running server and the migration tool. The async driver
is required at runtime; here we strip ``+asyncpg`` because Alembic uses a
sync engine internally.
"""
