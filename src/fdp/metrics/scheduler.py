...
The rollup logic (:mod:`fdp.metrics.aggregation`) is plain async functions
...
the start/stop shape of :class:`fdp.metrics.pipeline.MetricsPipeline`.
...
transactional and idempotent (:mod:`fdp.metrics.aggregation`), so a missed or
