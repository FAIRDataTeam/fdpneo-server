...
class ProfileSettings(BaseSettings):
    ...
    """If True, the lifespan handler applies the profile when
    ``profile_applied`` is empty. Off by default so dev startup never
    surprises an operator with a destructive operation."""

    path: Path | None = None
    """Filesystem location of the profile bundle (the directory containing
    ``profile.yaml``). Unset ⇒ the default DCAT bundle shipped inside the
    package (``fdpneo_server/profiles/default``)."""
...
class MigrationSettings(BaseSettings):
    """The one setting database migrations need, parsed exactly like ``Settings``.

    ``fdp db migrate`` (via ``migrations/env.py``) loads only this, so an
    installed wheel can migrate its database in environments where the rest
    of the configuration (triple store, OIDC) is absent.
    """
