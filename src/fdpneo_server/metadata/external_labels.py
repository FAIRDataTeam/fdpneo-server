...
Two collaborators, both used by :class:`fdpneo_server.metadata.labels.LabelResolver`:
...
:class:`fdpneo_server.config.RemoteLabelSettings`.
"""
...
from fdpneo_server.shared import negotiation
from fdpneo_server.shared.errors import UpstreamError
from fdpneo_server.shared.namespaces import DCT, FOAF, RDFS, SDO, SKOS
...
class ExternalLabelFetcher:
    ...
    timeout, and parsed through :mod:`fdpneo_server.shared.negotiation` (which blocks remote
    ...
    def _parse(self, body: bytes, content_type: str, *, base: str) -> Graph:
        ...
        Goes through :func:`fdpneo_server.shared.negotiation.parse` so the JSON-LD path keeps
