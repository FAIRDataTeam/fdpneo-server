...
* PATCH pipeline: parse → simulate locally → SHACL the post-update state →
  ...
  in :func:`fdpneo_server.metadata.patch.simulate_update`.
...
from fdpneo_server.metadata.meta import add_allowed_state_transitions
...
def build_ldp_router(
    *,
    ...
    def _navigation_links(iri: str, graph: Graph) -> str:
        ...
        :data:`~fdpneo_server.metadata.signposting.MAX_LINKS` while affordances always
