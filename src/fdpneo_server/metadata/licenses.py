...
The second of the two ODRL subsystems (the first is :mod:`fdpneo_server.metadata.policies`).
...
Surfaces mirror :mod:`fdpneo_server.metadata.policies` / :mod:`fdpneo_server.metadata.schemas`:
...
from fdpneo_server.shared.graphs import license_graph_uri, license_namespace, meta_graph_uri
from fdpneo_server.shared.namespaces import DCT, FDP_DEFAULT, FDP_METADATA_STATE, OWL
...
def predefined_license_shape_graph() -> Graph:
    ...
    :class:`~fdpneo_server.metadata.shacl.ShaclValidator` can resolve it.
    """
    graph = Graph()
