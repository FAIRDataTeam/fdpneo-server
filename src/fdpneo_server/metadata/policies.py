...
the runtime SHACL-shape admin (:mod:`fdpneo_server.metadata.schemas`). An Offer is no
...
(:func:`fdpneo_server.policy.parser.parse_offer`) on every write, and **enforced** by the
PDP — a record opts in via ``dct:rights`` and the existing
:class:`~fdpneo_server.policy.resolver.GraphBackedOfferResolver` fetches it by IRI.
...
Surfaces (mirror :mod:`fdpneo_server.metadata.schemas`):
...
from fdpneo_server.metadata.events import RecordDeleted, RecordModified
from fdpneo_server.metadata.states import MetadataState
from fdpneo_server.policy.parser import parse_offer
from fdpneo_server.shared.context import RequestContext
from fdpneo_server.shared.errors import BadRequest, Conflict, Forbidden, NotFound, SchemaViolation
from fdpneo_server.shared.graphs import meta_graph_uri, policy_graph_uri, policy_namespace
from fdpneo_server.shared.namespaces import DCT, FDP_METADATA_STATE, ODRL, OWL
