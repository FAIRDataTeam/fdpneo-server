...
Extracted from :mod:`fdpneo_server.metadata.profiles.applier` so the registry
(:mod:`fdpneo_server.metadata.profiles.registry`) can use it without importing
the applier, which would form a cycle (applier → registry → applier).

The convention here is the one documented on
:func:`fdpneo_server.metadata.profiles.apply_profile`:

* Schema / type CURIE: ``fdp:Repository`` → ``<fdp_namespace>Repository``;
  any prefix registered in :data:`fdpneo_server.shared.namespaces.PREFIXES`
...
from fdpneo_server.shared.graphs import schema_graph_uri
from fdpneo_server.shared.namespaces import PREFIXES, fdp_namespace
