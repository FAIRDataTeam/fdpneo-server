...
* No identifying field (IP, UA, subject, query text) has a column here.
  :class:`fdpneo_server.metrics.events.MetricSample` is the only shape consumed.
...
from fdpneo_server.metrics.events import MetricSample
...
def _status_class_sum(lower: int) -> Any:
    ...
    on-the-fly dashboard reader (:mod:`fdpneo_server.metrics.reader`).
    """
